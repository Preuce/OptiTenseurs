...
Cost MonoSide::solve(int s, int i){
    int ofs = s*s + i;
...
void MonoSide::restoreA(int s){
    int i = s;
    int ofs = size*s;

    A[ofs + i] = G[size*size + i];
    A[ofs + i+size/2] = G[size*size +i+size/2];
}

//TODO: might be errors
Cost MonoSide::contract(int x, int s){
    int i = s;
    int ofs = size*s;
...
void MonoSide::display_order(int s, int i){
    int ofs = s*s + i;
    switch(P[ofs]){
        case -1:
            break;
        case 2:
            cout << "|(" << O[ofs].first << ", " << O[ofs].second << ")|" << endl;
...
void MonoSide::get_size(char* Preamble){
...
void MonoSide::init(const char* file){
...
void MonoSide::execfile(const char* file){
...
void MonoSide::execdir(const char* dir){
